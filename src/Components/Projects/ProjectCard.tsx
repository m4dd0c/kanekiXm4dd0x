...
const ProjectCard = ({
  ...
  const mouseMove: React.MouseEventHandler<HTMLDivElement> = (e) => {
    ...
    const x = clientX - (left / 10 + width / 2);
    const y = clientY - (top / 1.5 + height / 2);
    setCPos({ left: x, top: y });
  };
      ...
      >
        <a href={data.link} target="_blank">
          <div
            ...
            id="cursor"
            style={{
              transform: `translate(-50%, -50%) translate(${cPos.left}px, ${cPos.top}px)`,
            }}
          >
            <h1 className="text-3xl bg-black text-white p-1 shadow-md">
              Live View
            </h1>
          </div>
          ...
          <div className="project-card-overlay tracking-wider text-white px-2 py-4">
            ...
            <h1 className="ml-5 text-sm text-red-50">{data.description}</h1>
          </div>
        </a>
      </motion.div>
